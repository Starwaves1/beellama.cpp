                ...
                case COMMON_SAMPLER_TYPE_DRY:
                    {
                        ...
                        if (config) {
                            common_sampler_chain_add_section(
                                    result,
                                    samplers,
                                    llama_sampler_init_dry(vocab, params.dry_multiplier, params.dry_base, params.dry_allowed_length, params.dry_penalty_last_n, c_breakers.data(), c_breakers.size()),
                                    llama_sampler_init_dry(vocab, reasoning.dry_multiplier, reasoning.dry_base, reasoning.dry_allowed_length, reasoning.dry_penalty_last_n, c_breakers.data(), c_breakers.size()),
                                    config);
                        } else {
                ...
                case COMMON_SAMPLER_TYPE_PENALTIES:
                    {
                        ...
                        if (config) {
                            common_sampler_chain_add_section(
                                    result,
                                    samplers,
                                    llama_sampler_init_penalties(llama_vocab_n_tokens(vocab), params.penalty_last_n, params.penalty_repeat, params.penalty_freq, params.penalty_present),
                                    llama_sampler_init_penalties(llama_vocab_n_tokens(vocab), reasoning.penalty_last_n, reasoning.penalty_repeat, reasoning.penalty_freq, reasoning.penalty_present),
                                    config);
                        } else {
...
// true when the chat template provides both reasoning delimiters
static bool common_sampler_has_reasoning_tags(const common_params_sampling & params) {
    return !params.reasoning_budget_start.empty() && !params.reasoning_budget_end.empty();
}

// reasoning budget sampler. Tracking/control modes observe reasoning state even when the token
// budget is unlimited, and per-section sampling overrides need the same state.
static llama_sampler * common_sampler_reasoning_budget_init(
    ...
    if (!common_sampler_has_reasoning_tags(params) ||
        !(need_rbudget_for_grammar ||
          params.reasoning_budget_tokens >= 0 ||
          params.reasoning_budget_tracking ||
...
static bool common_sampler_grammar_prefill(
        ...
        const common_params_sampling & params,
        const std::vector<llama_token> & prefill_tokens) {
    // output-format grammars are gated by the reasoning budget instead, so they must not be prefilled
    // with the generation prompt when the template has reasoning tags
    const bool skip_grammar_prefill =
        params.grammar.type == COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT &&
        common_sampler_has_reasoning_tags(params);

    if (!grmr || params.grammar_lazy || !common_grammar_needs_prefill(params.grammar) ||
        skip_grammar_prefill || prefill_tokens.empty()) {
        return false;
    }
...
struct common_sampler * common_sampler_init(const struct llama_model * model, struct common_params_sampling & params) {
    ...
    rbudget = common_sampler_reasoning_budget_init(vocab, params, grmr, prefill_tokens);
...
void common_sampler_configure_reasoning(
    ...
    gsmpl->rbudget = common_sampler_reasoning_budget_init(vocab, gsmpl->params, gsmpl->grmr, gsmpl->prefill_tokens);
}
...
void common_sampler_copy(const common_sampler * src, common_sampler * dst) {
    ...
    dst->prefill_tokens = src->prefill_tokens;
    dst->grmr_prefilled = src->grmr_prefilled;
...
// common_sampler::set_logits()'s full-logits branch (the else-branch). Used by
...
llama_token common_sampler_sample_from_logits(struct common_sampler * gsmpl, const float * logits, int n_vocab, bool grammar_first) {
    ...
    // budget -> [grammar] -> chain, the reasoning-end-on-EOG repair, plus grammar rejection-
    // resampling), with set_logits(ctx,idx) replaced by common_sampler_set_logits_raw(). It
    // intentionally omits two things that are inapplicable to replayed logits: llama_synchronize()
    // (the logits are caller-provided, not pending in any llama_context) and the backend-sampler
    // short-circuit (a replayed distribution never carries a backend-sampled token, and backend
    // sampling is incompatible with grammar/reasoning-budget anyway - see common_sampler_sample).
    //
    // KEEP IN SYNC with common_sampler_sample() above. The chain is applied through
    // common_sampler_apply_chain(), so per-section reasoning overrides are honoured for replayed
    // logits exactly as they are for live ones.
    GGML_ASSERT(logits != nullptr);
