...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        // any draft left over from the previous generation is never verified
        n_last[seq_id] = 0;
    ...
    bool set_state(llama_seq_id seq_id, const std::vector<uint8_t> & data) override {
        ...
        verify_h_rows[seq_id] = 0;
        i_last[seq_id] = -1;
        // the restored state belongs to a different round: drop the pending draft
        // length so the adaptive controller is not fed a stale n_draft
        n_last[seq_id] = 0;
        if (data.empty()) {
            std::fill(pending_h[seq_id].begin(), pending_h[seq_id].end(), 0.0f);
            return true;
